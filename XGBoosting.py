...
model = XGBRegressor(n_estimators=100, random_state=42)  # Using XGBoost Regressor
...
top_features = sorted_features[:17]
...
for combination in combinations_6_features:
    model_subset_features = XGBRegressor(n_estimators=100, random_state=42)
...
sorted_combinations = [comb for _, comb in sorted(zip(test_accuracies, combinations_6_features), reverse=True)]
